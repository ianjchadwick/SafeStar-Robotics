...
* Integrate simulated hazard source data to enable dynamic pathfinding adjustments.
...
2. **MCU Integration:** Grid representation, node struct, UART output for debugging and validation.
3. **Visualization:** OLED/LED matrix for real-time path and hazard output.
4. **Hazard Simulation:** Generate mock hazard data to test dynamic response.
...
* Dynamic hazard updates to verify SafeStar algorithm performance.
* UART logging for tracking path selection and algorithm decisions.

---

**SafeStar’s goal:**  
To compute a real-time path to the *closest safe exit*, balancing shortest path distance with maximum hazard avoidance.
